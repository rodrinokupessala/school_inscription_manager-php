 ...
 * @copyright  2022 Rodrino
 * @version    Release: 1.0
 */ 
define('url','192.168.0.198:8080');
...
function register($username, $password, $passwordConfirm, $email, $conn){
    ...
    try{
        ...
        $stmt->bindParam(':username1', decrypt($username));
        $stmt->execute();
        $result = $stmt->fetch(PDO::FETCH_ASSOC);
    }catch(PDOException $exception){ 
    ...
    if($error==false){
        ...
        try{
            ...
            logme($username,time(),"INSERT INTO users (username, password, email)
            VALUES (:username, :password, :email)","Error", $exception, "n/a");
            //header('Location:http://'.url.'/sign.php/?erro');
            echo $exception;
        }
...
function errorLogging($errorNum){
    ...
    if(isset($_GET["error"])){
        ...
        if($_GET["error"]=="16"){
            ...
            $error='<div class="col-lg-12"><div class="alert alert-warning">Possívelmente este código não existe!</div>';
        }
        return $error;
    }
}

function updatePassword($username, $email, $newPassword, $passwordConfirm, $token, $conn){
...
//https://stackoverflow.com/questions/10916284/how-to-encrypt-decrypt-data-in-php
 ...
 function encrypt($inputString){
    ...
    $token = $inputString;
    $encryption_key = 'AQWERFVBMMDKFLOTIGMMGM.#$?:/.,EHHDHRHRBRBFNNT236895';
...
function readLog(){
    $csvFile = file($_SERVER['DOCUMENT_ROOT']."/biblioteca/includes/log.csv", FILE_IGNORE_NEW_LINES | FILE_SKIP_EMPTY_LINES);
